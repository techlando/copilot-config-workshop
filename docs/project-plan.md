...
## 1. Project overview
Task Manager CLI is a small Node.js 20+ command-line application for managing personal tasks entirely in memory during runtime. Users can create, list, update, and delete tasks, then narrow results with filters and ordering options. The project is intentionally scoped for a workshop exercise, uses only built-in Node.js modules, and focuses on clean command parsing, predictable task lifecycle behavior, and testable domain logic.

## 2. User stories
1. As a user, I can create a task so that I can track work items.
Acceptance criteria:
- Given valid input, running a create command adds a task with title, description, status, priority, createdAt, and updatedAt.
...
- Each listed task includes id, title, status, priority, createdAt, and updatedAt.
...
5. As a user, I can filter listed tasks by status or priority so that I can focus on relevant items.
Acceptance criteria:
...
- Add validators for required title, valid status, and valid priority.
...
- Cover edge cases: empty list, invalid enums, missing id, combined filters, each sort mode.
